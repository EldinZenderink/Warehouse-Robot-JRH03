package asrs.Algorithms.Bpp;

import asrs.Models.Bin;
import asrs.Models.Product;

import java.util.ArrayList;
import java.util.Stack;

    /**
     * Created by ewart on 23-4-2015.
     */
public class Enumeratie implements Algoritme{

    private ArrayList<Product> order;//, bestCombination, leftArray;
    private Bin bin1, bin2;
    private boolean stop = false;
    private long difference;
    private ArrayList<Product> bestCombination;


        private Stack<Product> stack = new Stack<>();

        /** Store the sum of current elements stored in stack */

    public Enumeratie(Bin bin1, Bin bin2) {
        this.bin1 = bin1;
        this.bin2 = bin2;
        difference = 0;
        this.bestCombination = new ArrayList<>();
    }

    public void setOrder(ArrayList<Product> order){
        this.order = order;
    }



    public void startAlgo(){

    /**
     * this method contains a thread, this thread runs a for loop over a order array which contains packets
     * a random number generator and a equation to check if the number is even or odd is used to determine
     * if the packet in the the order array is supposed to go left or right.
     */


        long lStartTime = System.currentTimeMillis();
        ArrayList<Product> tempOrder = new ArrayList<>();

        for(Product pInOrder : order){
            tempOrder.add(pInOrder);
        }
        Bin highestBin;
        Bin lowestBin;
        while(!tempOrder.isEmpty()) {
            if (bin1.getBinCapacityHeight() >= bin2.getBinCapacityHeight()) {
                highestBin = bin1;
                lowestBin = bin2;
            } else {
                highestBin = bin2;
                lowestBin = bin1;

            }
            BestCombo bo = new BestCombo(tempOrder.size(), highestBin.getBinCapacityHeight());
            bestCombination = bo.calculateTarget(tempOrder, 0, tempOrder.size());
            System.out.print(" Beste StackId: " + bo.getBestStackId());
            System.out.print(" combo: {");
            for(Product p: bestCombination) {
                System.out.print(p.getSize() + ",");
            }
            System.out.println("}");

            if(calculateStackHeight(highestBin.getProducts()) == highestBin.getBinCapacityHeight() && calculateStackHeight(bestCombination) == highestBin.getBinCapacityHeight()) highestBin.emptyBin();
            for (int x = 0; x < bo.getBestStackId();x++ ) {
//                        if(parentscreen.getDelay() >= 50){
//                            parentscreen.delay(false);
//                            SwingUtilities.invokeLater(new Runnable() {
//                                @Override
//                                public void run() {
//                                    draw.repaint();
//                                }
//                            });
//                        }
                if(bestCombination.contains(tempOrder.get(x))) {
                    highestBin.addPacket(tempOrder.get(x), "Enumeratie");
                    if(highestBin.equals(bin2)) {
                        tempOrder.get(x).setGoLeft(false);
                    }
                } else {
                    if(lowestBin.getBinCapacityLeft() < tempOrder.get(x).getSize()) {
                        lowestBin.emptyBin();
                    }
                    lowestBin.addPacket(tempOrder.get(x), "Enumeratie");
                    if(lowestBin.equals(bin2)) {
                        tempOrder.get(x).setGoLeft(false);
                    }
                }

            }
            for(int x = 0; x < bo.getBestStackId();x++) {
                tempOrder.remove(0);

            }

        }
        long lEndTime = System.currentTimeMillis();
        difference = lEndTime - lStartTime;

        stop = false;
    }

    public void stopAlgo(){
        stop = true;
    }

    public int calculateStackHeight(ArrayList<Product> stack){
        int stackHeight = 0;
        for( Product p: stack) {
            stackHeight += p.getSize();
        }
        return stackHeight;
    }


}
